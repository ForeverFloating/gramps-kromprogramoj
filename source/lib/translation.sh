#!/usr/bin/env sh
################################################################
# script for automatically translating a sentence
################################################################
DEBUG=
#DEBUG=1
...
cook=$(find . -maxdepth 1 _translation.jar -mmin -15 2>/dev/null)
...
if [ -z "$cook" ]
then
  curl -c _translation.jar -A 'Mozilla/5.0 (X11; Linux x86_64; rv:68.0) Gecko/20100101 Firefox/68.0' \
  'https://translate.google.com' -o /dev/null 2>/dev/null
fi

MSG0=$(curl -b _translation.jar -A 'Mozilla/5.0 (X11; Linux x86_64; rv:68.0) Gecko/20100101 Firefox/68.0' \
